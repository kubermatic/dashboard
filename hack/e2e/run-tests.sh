...
export CYPRESS_KUBECONFIG_ENCODED="$(kind get kubeconfig --name="$KIND_CLUSTER_NAME" | yq r - -j -I=0 | sed 's/127.0.0.1.*/kubernetes.default.svc.cluster.local./' | base64 -w0)"
# TODO sed 's/127.0.0.1.*/kubernetes.default.svc.cluster.local./' as alternative
