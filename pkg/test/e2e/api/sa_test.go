...
func TestTokenAccessForProject(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err = apiRunnerWithSAToken.AddProjectUser(project.ID, "roxy-admin@kubermatic.com", "roxy-admin", "viewers")
			switch tc.group {
			...
			case rbac.ProjectManagerGroupNamePrefix:
				newSAproject, err := apiRunnerWithSAToken.CreateProjectBySA(rand.String(10), []string{"roxy-admin@kubermatic.com", "roxy@kubermatic.com"})
				if err != nil {
