...
export class AnexiaService {
  ...
  getVLans(projectID: string, clusterID: string): Observable<AnexiaVlan[]> {
    const url = `${this._newRestRoot}/projects/${projectID}/clusters/${clusterID}/providers/anexia/vlans`;
    return this._httpClient.get<AnexiaVlan[]>(url);
  }

  getTemplates(projectID: string, clusterID: string, location: string): Observable<AnexiaTemplate[]> {
