...
func TestCreateClusterEndpoint(t *testing.T) {
	...
	ciliumVersion := cni.GetDefaultCNIPluginVersion(kubermaticv1.CNIPluginTypeCilium)
	...
	testcases := []struct {
		...
		// scenario 1
		{
			...
			ExistingKubermaticObjs: test.GenDefaultKubermaticObjects(test.GenTestSeed()),
			ProjectToSync:          test.GenDefaultProject().Name,
			ExistingAPIUser:        test.GenDefaultAPIUser(),
		},
		// scenario 2
		{
			...
			ExpectedResponse: fmt.Sprintf(`{"id":"%%s","name":"keen-snyder","annotations":{"kubermatic.io/initial-application-installations-request":"[]"},"creationTimestamp":"0001-01-01T00:00:00Z","type":"kubernetes","spec":{"cloud":{"dc":"fake-dc","fake":{}},"version":"%s","oidc":{},"enableUserSSHKeyAgent":true,"kubernetesDashboard":{"enabled":true},"containerRuntime":"containerd","clusterNetwork":{"ipFamily":"IPv4","services":{"cidrBlocks":["10.240.16.0/20"]},"pods":{"cidrBlocks":["172.25.0.0/16"]},"nodeCidrMaskSizeIPv4":24,"dnsDomain":"cluster.local","proxyMode":"nftables","ipvs":{"strictArp":true},"nodeLocalDNSCacheEnabled":true,"konnectivityEnabled":true},"cniPlugin":{"type":"cilium","version":"%s"},"exposeStrategy":"NodePort"},"status":{"version":"","url":"","externalCCMMigration":"Unsupported"}}`, version, ciliumVersion),
		...
		},
		{
			...
			ExpectedResponse: fmt.Sprintf(`{"id":"%%s","name":"keen-snyder","annotations":{"kubermatic.io/initial-application-installations-request":"[]"},"creationTimestamp":"0001-01-01T00:00:00Z","type":"kubernetes","spec":{"cloud":{"dc":"audited-dc","fake":{}},"version":"%s","oidc":{},"enableUserSSHKeyAgent":true,"kubernetesDashboard":{"enabled":true},"auditLogging":{"enabled":true},"containerRuntime":"containerd","clusterNetwork":{"ipFamily":"IPv4","services":{"cidrBlocks":["10.240.16.0/20"]},"pods":{"cidrBlocks":["172.25.0.0/16"]},"nodeCidrMaskSizeIPv4":24,"dnsDomain":"cluster.local","proxyMode":"nftables","ipvs":{"strictArp":true},"nodeLocalDNSCacheEnabled":true,"konnectivityEnabled":true},"cniPlugin":{"type":"cilium","version":"%s"},"exposeStrategy":"NodePort"},"status":{"version":"","url":"","externalCCMMigration":"Unsupported"}}`, version, ciliumVersion),
			RewriteClusterID: true,
			HTTPStatus:       http.StatusCreated,
			ProjectToSync:    test.GenDefaultProject().Name,
			ExistingKubermaticObjs: test.GenDefaultKubermaticObjects(
	...
	for _, tc := range testcases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			ep, clientsSets, err := test.CreateTestEndpointAndGetClients(*tc.ExistingAPIUser, nil, []ctrlruntimeclient.Object{}, nil, kubermaticObj, &dummyKubermaticConfiguration, hack.NewTestRouting)
			if err != nil {
				t.Fatalf("failed to create test endpoint: %v", err)
			}
			...
			var actualCluster *apiv1.Cluster
			...
			// Verify encryption secret is created in master client, not seed client
			if tc.VerifyEncryptionSecret && actualCluster != nil {
				ctx := context.Background()
				secretName := fmt.Sprintf("encryption-key-cluster-%s", actualCluster.ID)

				// Verify the encryption secret was created in the MASTER client
				masterSecret := &corev1.Secret{}
				if err := clientsSets.FakeMasterClient.Get(ctx, types.NamespacedName{
					Namespace: resources.KubermaticNamespace,
					Name:      secretName,
				}, masterSecret); err != nil {
					t.Fatalf("encryption secret should be created in master client: %v", err)
				}

				// Verify the secret has the correct data
				if string(masterSecret.Data["key"]) != tc.ExpectedEncryptionKey {
					t.Errorf("expected secret data 'key' to be '%s', got '%s'", tc.ExpectedEncryptionKey, string(masterSecret.Data["key"]))
				}

				// Verify the encryption secret was NOT created in the SEED client
				seedSecret := &corev1.Secret{}
				err = clientsSets.FakeSeedClient.Get(ctx, types.NamespacedName{
					Namespace: resources.KubermaticNamespace,
					Name:      secretName,
				}, seedSecret)
				if err == nil {
					t.Errorf("encryption secret should NOT be created in seed client, but it was found")
				}
			}
		})
	}
...
func TestPatchCluster(t *testing.T) {
	...
	testcases := []struct {
		...
		// scenario 3
		{
			...
			ExpectedResponse: `{"id":"keen-snyder","name":"clusterAbc","creationTimestamp":"2013-02-03T19:54:00Z","type":"kubernetes","spec":{"cloud":{"dc":"fake-dc","fake":{}},"version":"9.11.3","oidc":{},"enableUserSSHKeyAgent":false,"kubernetesDashboard":{"enabled":true},"containerRuntime":"containerd","clusterNetwork":{"ipFamily":"IPv4","services":{"cidrBlocks":["5.6.7.8/8"]},"pods":{"cidrBlocks":["1.2.3.4/8"]},"nodeCidrMaskSizeIPv4":24,"dnsDomain":"cluster.local","proxyMode":"nftables","ipvs":{"strictArp":true},"nodeLocalDNSCacheEnabled":true},"cniPlugin":{"type":"canal","version":"v3.31"},"exposeStrategy":"NodePort"},"status":{"version":"9.9.9","url":"https://w225mx4z66.asia-east1-a-1.cloud.kubermatic.io:31885","externalCCMMigration":"Unsupported"}}`,
			cluster:          "keen-snyder",
			HTTPStatus:       http.StatusOK,
			project:          test.GenDefaultProject().Name,
			ExistingAPIUser:  test.GenDefaultAPIUser(),
...
func TestGetCluster(t *testing.T) {
	...
	testcases := []struct {
		...
		// scenario 1
		{
			...
			Body:             ``,
			ExpectedResponse: `{"id":"defClusterID","name":"defClusterName","creationTimestamp":"2013-02-03T19:54:00Z","type":"kubernetes","spec":{"cloud":{"dc":"private-do1","fake":{}},"version":"9.9.9","oidc":{},"enableUserSSHKeyAgent":false,"containerRuntime":"containerd","clusterNetwork":{"ipFamily":"IPv4","services":{"cidrBlocks":["5.6.7.8/8"]},"pods":{"cidrBlocks":["1.2.3.4/8"]},"nodeCidrMaskSizeIPv4":24,"dnsDomain":"cluster.local","proxyMode":"nftables","ipvs":{"strictArp":true}},"cniPlugin":{"type":"canal","version":"v3.31"},"exposeStrategy":"NodePort"},"status":{"version":"9.9.9","url":"https://w225mx4z66.asia-east1-a-1.cloud.kubermatic.io:31885","externalCCMMigration":"Unsupported"}}`,
