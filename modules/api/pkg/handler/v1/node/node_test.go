...
func TestCreateNodeDeployment(t *testing.T) {
	...
	testcases := []struct {
		...
		// scenario 1
		{
			...
			Body:             `{"spec":{"replicas":1,"template":{"cloud":{"digitalocean":{"size":"s-1vcpu-1gb","backups":false,"ipv6":false,"monitoring":false,"tags":[]}},"operatingSystem":{"ubuntu":{"distUpgradeOnBoot":false}},"network":{"cidr":"","gateway":"","dns":{"servers":null},"ipFamily":"IPv4"},"network":{"cidr":"","gateway":"","dns":{"servers":null},"ipFamily":"IPv4"}}}}`,
...
func TestGetNodeDeployment(t *testing.T) {
	...
	testcases := []struct {
		...
		// scenario 2
		{
			Name:            "scenario 2: get node deployment that belong to the given cluster and has dynamic config set up",
			HTTPStatus:      http.StatusOK,
			ClusterIDToSync: test.GenDefaultCluster().Name,
			ProjectIDToSync: test.GenDefaultProject().Name,
			ExistingKubermaticObjs: test.GenDefaultKubermaticObjects(
...
func TestListNodeDeploymentNodes(t *testing.T) {
	...
	testcases := []struct {
		...
		// scenario 2
		{
			Name:            "scenario 2: the admin John can list any nodes that belong to the given node deployment",
			HTTPStatus:      http.StatusOK,
			ClusterIDToSync: test.GenDefaultCluster().Name,
			ProjectIDToSync: test.GenDefaultProject().Name,
			ExistingKubermaticObjs: test.GenDefaultKubermaticObjects(
...
func TestDeleteNodeDeployment(t *testing.T) {
	...
	testcases := []struct {
		...
		// scenario 2
		{
			Name:            "scenario 2: the admin John can delete any cluster node",
			HTTPStatus:      http.StatusOK,
			NodeIDToDelete:  "venus",
			ClusterIDToSync: test.GenDefaultCluster().Name,
			ProjectIDToSync: test.GenDefaultProject().Name,
		...
		// scenario 3
		{
			Name:            "scenario 3: the user John can delete Bob's cluster node",
			HTTPStatus:      http.StatusForbidden,
			NodeIDToDelete:  "venus",
			ClusterIDToSync: test.GenDefaultCluster().Name,
			ProjectIDToSync: test.GenDefaultProject().Name,
...
func TestNodeDeploymentMetrics(t *testing.T) {
	...
	testcases := []struct {
		...
		// scenario 2
		{
			Name:            "scenario 2: the admin John can get any metrics for the node deployment nodes",
			HTTPStatus:      http.StatusOK,
			ClusterIDToSync: test.GenDefaultCluster().Name,
			ProjectIDToSync: test.GenDefaultProject().Name,
			ExistingKubermaticObjs: test.GenDefaultKubermaticObjects(
