...
func SetPresetProviderEnabled(p *kubermaticv1.Preset, providerType kubermaticv1.ProviderType, enabled bool) {
...
func PresetHasProvider(p *kubermaticv1.Preset, providerType kubermaticv1.ProviderType) (bool, reflect.Value) {
	provider := getProviderValue(&p.Spec, providerType)
	return provider.IsValid() && !provider.IsZero(), provider
}

func GetPresetProviderList(p *kubermaticv1.Preset) []kubermaticv1.ProviderType {
	existingProviders := []kubermaticv1.ProviderType{}
	for _, provType := range kubermaticv1.SupportedProviders {
		if hasProvider, _ := PresetHasProvider(p, provType); hasProvider {
...
func GetProviderPreset(p *kubermaticv1.Preset, providerType kubermaticv1.ProviderType) *kubermaticv1.ProviderPreset {
	hasProvider, providerField := PresetHasProvider(p, providerType)
	if !hasProvider {
		return nil
	}
...
func ValidatePreset(p *kubermaticv1.Preset, providerType kubermaticv1.ProviderType) error {
	hasProvider, providerField := PresetHasProvider(p, providerType)
	if !hasProvider {
...
func IsPresetProviderEnabled(p *kubermaticv1.Preset, provider kubermaticv1.ProviderType) bool {
	presetProvider := GetProviderPreset(p, provider)
	return presetProvider != nil && presetProvider.IsEnabled()
}

func OverridePresetProvider(p *kubermaticv1.Preset, providerType kubermaticv1.ProviderType, newPreset *kubermaticv1.Preset) *kubermaticv1.Preset {
...
func RemovePresetProvider(p *kubermaticv1.Preset, providerType kubermaticv1.ProviderType) *kubermaticv1.Preset {
	provider := getProviderValue(&p.Spec, providerType)
