...
func PatchEndpoint(
	...
	if err := defaulting.DefaultClusterSpec(ctx, &newInternalCluster.Spec, newInternalCluster.Labels, defaultingTemplate, seed, config, cloudProvider); err != nil {
		return nil, err
	}
...
func setupEncryptionSpec(cluster *kubermaticv1.Cluster) error {
	...
	// Initialize secretbox configuration
	...
	// Reference the cluster's encryption secret in secretbox keys
	secretName := getEncryptionSecretName(cluster.Name)
...
func handleEncryptionAtRestUpdate(ctx context.Context, masterClient ctrlruntimeclient.Client, existingCluster, updatedCluster *kubermaticv1.Cluster, encryptionAtRest *apiv1.EncryptionAtRestSpec) error {
	...
	// Enable encryption for the first time or after it was previously disabled
