...
import {Openstack, Provider} from '../../utils/provider';
import {View} from '../../utils/view';
import {WizardStep} from '../../utils/wizard';

describe('OpenStack Provider', () => {
  const preset = Mocks.enabled() ? Preset.Mock : Preset.OpenStack;
  ...
  beforeEach(() => {
    if (Mocks.enabled()) {
      Mocks.register(Provider.OpenStack);
    }
  });
  ...
  it('should create a new cluster', () => {
    WizardPage.getProviderBtn(Provider.OpenStack).click();
    WizardPage.getDatacenterBtn(Openstack.Syseleven).click();
