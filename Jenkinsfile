  ...
  )
  {
    node ('buildpod') {
        try {
            ...
            stage('setup workdir'){
                container('golang') {
                    sh("ln -s `pwd` /go/src/github.com/kubermatic/dashboard-v2")
                    sh("cd /go/src/github.com/kubermatic/dashboard-v2")
                    checkout scm
                }
            }
...
def buildPipeline(dockerTags, deployTag, stageSystem) {
    ...
    stage('Install deps'){
        container('node') {
           sh("cd /go/src/github.com/kubermatic/dashboard-v2 && make install")
        }
    }
    stage('Build dist'){
        container('node') {
...
/*    stage('Test'){
    ...
    stage('Build docker image'){
        container('docker') {
            sh("cd /go/src/github.com/kubermatic/dashboard-v2 && make TAGS='${dockerTags}' docker-build")
        }
    }
    stage('Push'){
        container('docker') {
                ...
                stage('Deploy Staging'){
                    container('docker') {
                        withCredentials([string(credentialsId: 'kubeconfig-dev', variable: 'KUBECONFIG')]) {
                            sh("echo '$KUBECONFIG'>kubeconfig && kubectl --kubeconfig=kubeconfig set image deployment/kubermatic-ui-v1 webserver=kubermatic/ui-v2:'${deployTag}' --namespace=kubermatic")
                        }
