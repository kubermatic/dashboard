...
export function objectFromForm(
  ...
  for (const i in control.value) {
    if (
      control.value[i].key !== '' &&
      (control.value[i].value !== '' || allowEmptyValue)
    ) {
...
export function filterArrayOptions(
  value: string,
  field: string,
  options: any
): any {
...
export function filterObjectOptions(
  value: string,
  field: string,
  options: any
): any {
  ...
  Object.keys(options).forEach(key => {
    result[key] = options[key].filter(option =>
      option[field].toLowerCase().includes(filterValue)
    );
  });
  return result;
