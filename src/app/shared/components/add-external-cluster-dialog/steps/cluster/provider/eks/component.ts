...
import {EKSCluster} from '@shared/entity/external-cluster-model';
import {MatTableDataSource} from '@angular/material/table';
import {MatPaginator} from '@angular/material/paginator';
import {UserService} from '@core/services/user';

@Component({
  selector: 'km-eks-cluster',
  templateUrl: './template.html',
  styleUrls: ['./style.scss'],
})
export class EKSClusterComponent implements OnInit, OnDestroy {
  @Input() projectID: string;
  isInitialized = false;
  clusters: EKSCluster[] = [];
  dataSource = new MatTableDataSource<EKSCluster>();
  columns: string[] = ['name', 'region', 'import'];
  @ViewChild(MatPaginator, {static: true}) private readonly _paginator: MatPaginator;
  private _selected: EKSCluster;
  ...
  isSelected(cluster: EKSCluster): boolean {
    return this._selected === cluster;
  }

  select(cluster: EKSCluster): void {
    ...
    this._externalClusterService.externalCluster.cloud.eks.region = this._selected.region;
  }
}
