...
} from '../../testing/fake-data/clusterWithMachineNetworks.fake';
import {NoIpsLeftValidator} from './no-ips-left.validator';

describe('NoIPsLeftwValidator', () => {
  it('enough ips left', () => {
    const validatorFn = NoIpsLeftValidator(
      fakeClusterWithMachineNetwork().spec.machineNetworks,
      0
    );
  ...
  it('gateway ip is in ip range', () => {
    const validatorFn = NoIpsLeftValidator(
      fakeGatewayInCidr().spec.machineNetworks,
      0
    );
  ...
  it('gateway ip is not in ip range', () => {
    const validatorFn = NoIpsLeftValidator(
      fakeGatewayNotInCidr().spec.machineNetworks,
      0
    );
    const control = {value: 6};
    const result = validatorFn(control as AbstractControl);
