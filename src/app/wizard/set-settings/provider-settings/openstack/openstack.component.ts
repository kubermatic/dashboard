...
export class OpenstackClusterSettingsComponent implements OnInit, OnDestroy {
  ...
  loadTenants(): void {
    if (this.openstackSettingsForm.controls.username.value === '' ||
        ...
        this.openstackSettingsForm.controls.domain.value === '' || this.tenants.length > 0 || this.network.length > 0 ||
  ...
  loadOptionalSettings(): void {
    if (this.openstackSettingsForm.controls.username.value === '' ||
        ...
        this.openstackSettingsForm.controls.tenant.value === '' || this.network.length > 0 ||
        this.securityGroup.length > 0) {
      return;
    }
            ...
            .subscribe((networks: OpenstackNetwork[]) => {
              ...
              if (this.floatingIpPool.length > 0 && this.openstackSettingsForm.controls.floatingIpPool.value !== '0') {
                this.openstackSettingsForm.controls.floatingIpPool.setValue(
                    this.cluster.spec.cloud.openstack.floatingIpPool);
              }
            }));
