...
export class AWSProviderBasicComponent extends BaseFormValidator implements OnInit, OnDestroy {
    ...
    this.form.valueChanges.pipe(takeUntil(this._unsubscribe))
        .subscribe(
            _ => this._presets.enablePresets(
                Object.values(this._clusterService.cluster.spec.cloud.aws).every(value => !value)));
  ...
  private _hasRequiredCredentials(): boolean {
    return !!this._clusterService.cluster.spec.cloud.aws && !!this._clusterService.cluster.spec.cloud.aws.accessKeyId &&
  ...
  private _setDefaultVPC(vpcs: AWSVPC[]): void {
    ...
    this.vpcLabel = defaultVPC ? VPCState.Ready : VPCState.Empty;
    this._cdr.detectChanges();
  }
