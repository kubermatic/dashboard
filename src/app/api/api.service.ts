...
export class ApiService {
  ...
  getClusters(dc: string): Observable<ClusterEntity[]> {
    let url = `${this.restRoot}/dc/${dc}/cluster`;
  ...
  setCloudProvider(clusterModel: ClusterModel, node_dc: string): Observable<ClusterEntity> {
    let url = `${this.restRoot}/dc/${clusterModel.dc}/cluster/${clusterModel.cluster}/cloud`;
  ...
  getKubeconfig(clusterModel: ClusterModel, authorization_token: string): Observable<string> {
    let url = `${this.restRoot}/dc/${clusterModel.dc}/cluster/${clusterModel.cluster}/kubeconfig?token=${authorization_token}`;
  ...
  getClusterNode(clusterModel: ClusterModel, node_name: string): Observable<NodeEntity> {
    let url = `${this.restRoot}/dc/${clusterModel.dc}/cluster/${clusterModel.cluster}/node/${node_name}`;
  ...
  getKubernetesNode(clusterModel: ClusterModel, node_name: string) {
    let url = `${this.restRoot}/dc/${clusterModel.dc}/cluster/${clusterModel.cluster}/k8s/nodes/${node_name}`;
